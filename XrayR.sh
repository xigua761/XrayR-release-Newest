...
#        return 0
#    fi
    bash <(curl -Ls https://raw.githubusercontent.com/mottzz87/XrayR-release/master/install.sh) $version
    if [[ $? == 0 ]]; then
...
update_shell() {
    wget -O /usr/bin/XrayR -N --no-check-certificate https://raw.githubusercontent.com/mottzz87/XrayR-release/master/XrayR.sh
    if [[ $? != 0 ]]; then
        echo ""
...
--- https://github.com/mottzz87/XrayR ---
  ${green}0.${plain} 修改配置
————————————————
